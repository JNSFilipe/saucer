...
#include "wpe.navigation.impl.hpp"
...
namespace saucer
{
    ...
    void native::setup<event::permission>(impl *self)
    {
        ...
        auto callback = [](WebKitWebView *, WebKitPermissionRequest *raw, impl *self)
        {
            ...
            // WPE doesn't have clipboard or pointer lock permission requests
            static constexpr auto mappings = std::array<gboolean (*)(gpointer, permission::type &), 4>{
            ...
            auto req = std::make_shared<request>(request::impl{
                ...
                .url     = self->url().value_or(saucer::url{}),
                .type    = type,
            });
